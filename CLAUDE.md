...
- **Errors** — typed hierarchy under `UiPathError`. ErrorFactory maps HTTP status codes to specific types (AuthenticationError, NotFoundError, etc.). See `src/core/errors/`.
...
- **Use enums** for fixed sets of values; don't leave them as raw strings/numbers.
...
- CI runs on PRs: install → test with coverage → build (`.github/workflows/pr-checks.yml`).
