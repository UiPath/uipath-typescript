...
- `sdk.logout()` - Logout and clear all authentication state (requires re-initialization to authenticate again). By default this is local-only: the Automation Cloud session is untouched and a subsequent sign-in completes silently. Pass `sdk.logout({ endSession: true })` to also terminate the Automation Cloud session — the browser is redirected to the Identity end-session endpoint, so the next sign-in prompts for credentials. Pass `postLogoutRedirectUri` to return the user to your app afterwards — the value must match a redirect URI registered on your External Application **exactly** (a trailing-slash difference is a mismatch), typically your app's login redirect URI. When omitted or mismatched, the user lands on the Automation Cloud portal. **Include the `openid` scope in your SDK configuration** so the SDK can send the OIDC ID token as `id_token_hint` — this lets Identity skip its logout-confirmation prompt, and it is required for the return redirect to be honored. Without it the SDK logs a warning, Identity asks the user to confirm sign-out, and the redirect is ignored.
