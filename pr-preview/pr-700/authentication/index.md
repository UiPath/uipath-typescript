...
By default users see the UiPath account sign-in screen. Set `forceSso: true` to send them straight to the organization's configured identity provider instead.
...
This uses the organization id, which the platform and `uipath.json` supply for you. If you instead pass configuration directly to the constructor, set `orgName` to the organization id, otherwise `forceSso` is ignored and a warning is logged.
