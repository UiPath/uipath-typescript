...
### Signing in through the organization's identity provider

By default the SDK does not tell Identity which organization the sign-in is for — Identity resolves that from the Client ID. Users are shown the UiPath account sign-in screen, and those belonging to an organization with SSO can continue from there to their provider.

If your organization signs everyone in through SSO and you want users taken straight to your identity provider, set `forceSso` and pass the **organization id** as `orgName`:

```
const sdk = new UiPath({
  baseUrl: 'https://cloud.uipath.com',
  orgName: '3aa10965-a82d-4d9e-8366-0eff8e87bf7a', // organization id, not the name
  tenantName: 'your-tenant',
  clientId: 'your-client-id',
  redirectUri: 'http://localhost:3000',
  scope: 'OR.Users OR.Assets',
  forceSso: true,
});
```

Only for organizations where everyone signs in through SSO

`forceSso` sends users directly to the organization's identity provider. Anyone who signs in with a UiPath account instead — a local account, or an external collaborator — does not exist in that provider and will be rejected by it. Leave `forceSso` off if your organization allows both.

Requires the organization id

Identity resolves the organization from its id, so `orgName` must be the GUID. If it is an organization name, `forceSso` is ignored and a warning is logged.
...
1. In UiPath Cloud: **Admin** → **External Applications**
1. Click **Add Application** → **Confidential Application**
1. Configure:
   - **Name**: Your app name
...
1. Save and copy the **Client ID** and the **Client Secret** — you will not be shown the Client Secret again
1. Follow the [official UiPath documentation](https://docs.uipath.com/automation-cloud/automation-cloud/latest/api-guide/accessing-uipath-resources-using-external-applications) on how to request client-credentials tokens for a confidential app with application scopes
1. Pass this token as your `secret`, as shown in the Secret-based Authentication example above
