...
> **getCatalogByName**(`name`: `string`, `options?`: `TaskCatalogGetByNameOptions`): `Promise`\<`TaskCatalogGetResponse`>
...
The matching task catalog [TaskCatalogGetResponse](../TaskCatalogGetResponse/)

#### Example

```
const catalog = await tasks.getCatalogByName("Invoices", { folderId: <folderId> });
```
