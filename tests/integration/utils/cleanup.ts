...
export async function cleanupTestTask(
  ...
  try {
    ...
    await retryWithBackoff(async () => {
      ...
      try {
        await sdk.tasks.unassign({ taskIds: [taskId] });
      } catch {
...
export async function cleanupTestEntityRecords(
  ...
  try {
    const sdk = getTestSDK();
    await retryWithBackoff(async () => {
      await sdk.entities.deleteById(entityId, recordIds);
    });
...
export async function cleanupTestProcessInstance(
  ...
  try {
    const sdk = getTestSDK();
    await retryWithBackoff(async () => {
      await sdk.maestro.processes.instances.cancel(instanceId, folderKey || '');
    });
...
export async function cleanupTestCaseInstance(
  ...
  try {
    const sdk = getTestSDK();
    await retryWithBackoff(async () => {
      await sdk.maestro.cases.instances.close(caseId, folderKey || '');
    });
