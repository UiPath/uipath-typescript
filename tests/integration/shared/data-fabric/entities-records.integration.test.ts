...
describe.each(modes)('Data Fabric Entities Records - Integration Tests [%s]', (mode) => {
  ...
  describe('upsertMultiEntityTransaction', () => {
    ...
    it('should write a parent and its children in one transaction', async () => {
      ...
      const result = await entities.upsertMultiEntityTransaction(
        { name: treeEntityName },
      ...
      expect(result.transaction.entityName).toBe(treeEntityName);
      expect(result.transaction.op).toBe(EntityMultiEntityWriteOperation.Insert);
      expect(result.transaction.id).toBe(result.Id);
      // Root plus one child
      expect(result.transaction.totalRecordsAffected).toBe(2);

      expect(result.transaction.members).toHaveLength(1);
    ...
    it('should update the root and insert another child in the same transaction', async () => {
      ...
      const seed = await entities.upsertMultiEntityTransaction(
        { name: treeEntityName },
        {
      ...
      expect(result.transaction.op).toBe(EntityMultiEntityWriteOperation.Update);
      expect(result.transaction.members[0].op).toBe(EntityMultiEntityWriteOperation.Insert);
      expect(result.transaction.totalRecordsAffected).toBe(2);

      registerResource('entityRecords', {
        entityId: treeChildMetadata.id,
        recordIds: [result.transaction.members[0].id],
      });
    }, 60_000);

    it('should return typed transaction fields and drop the composite envelope keys', async () => {
      const { entities } = getServices();
      ...
      expect(typeof result.transaction.entityName).toBe('string');
      expect(typeof result.transaction.affectedRows).toBe('number');
      expect(typeof result.transaction.totalRecordsAffected).toBe('number');
      expect(Array.isArray(result.transaction.members)).toBe(true);

      // Every node reports its own outcome
      expect(typeof result.transaction.members[0].entityName).toBe('string');
      expect(typeof result.transaction.members[0].affectedRows).toBe('number');

      // Composite-only envelope keys are dropped
    ...
    it('should write a tree through the bound method on the entity', async () => {
      ...
      const result = await treeMetadata.upsertMultiEntityTransaction({
        ...rootData,
