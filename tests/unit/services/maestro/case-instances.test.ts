...
describe('CaseInstancesService', () => {
  ...
  beforeEach(async () => {
    ...
    service = new CaseInstancesService(instance);
  });
  ...
  describe('getById', () => {
    it('should return case instance by ID with operation methods and transformed properties', async () => {
      ...
      mockApiClient.get.mockResolvedValueOnce(mockRawApiResponse).mockResolvedValueOnce(mockCaseJsonResponse);
      ...
      expect(mockApiClient.get).toHaveBeenCalledWith(MAESTRO_ENDPOINTS.CASES.GET_CASE_JSON(instanceId), {
        headers: expect.objectContaining({
          [FOLDER_KEY]: folderKey,
        }),
      });
    ...
    it('should handle case JSON without caseAppConfig', async () => {
      ...
      mockApiClient.get.mockResolvedValueOnce(mockRawApiResponse).mockResolvedValueOnce(mockCaseJson);
    ...
    it('should handle initial API error', async () => {
      ...
      await expect(
        service.getById(MAESTRO_TEST_CONSTANTS.CASE_INSTANCE_ID, MAESTRO_TEST_CONSTANTS.FOLDER_KEY),
  ...
  describe('close', () => {
    it('should close case instance successfully', async () => {
      ...
      expect(mockApiClient.post).toHaveBeenCalledWith(MAESTRO_ENDPOINTS.INSTANCES.CANCEL(instanceId), options, {
        headers: expect.objectContaining({
          [FOLDER_KEY]: folderKey,
        }),
      });
    ...
    it('should handle API errors', async () => {
      ...
      await expect(
        service.close(MAESTRO_TEST_CONSTANTS.CASE_INSTANCE_ID, MAESTRO_TEST_CONSTANTS.FOLDER_KEY),
      ).rejects.toThrow(TEST_CONSTANTS.ERROR_MESSAGE);
    });
  });
  ...
  describe('pause', () => {
    it('should pause case instance successfully', async () => {
      ...
      expect(mockApiClient.post).toHaveBeenCalledWith(MAESTRO_ENDPOINTS.INSTANCES.PAUSE(instanceId), options, {
        headers: expect.objectContaining({
          [FOLDER_KEY]: folderKey,
        }),
      });
    ...
    it('should handle API errors', async () => {
      ...
      await expect(
        service.pause(MAESTRO_TEST_CONSTANTS.CASE_INSTANCE_ID, MAESTRO_TEST_CONSTANTS.FOLDER_KEY),
      ).rejects.toThrow(TEST_CONSTANTS.ERROR_MESSAGE);
    });
  });
  ...
  describe('resume', () => {
    it('should resume case instance successfully', async () => {
      ...
      expect(mockApiClient.post).toHaveBeenCalledWith(MAESTRO_ENDPOINTS.INSTANCES.RESUME(instanceId), options, {
        headers: expect.objectContaining({
          [FOLDER_KEY]: folderKey,
        }),
      });
    ...
    it('should handle API errors', async () => {
      ...
      await expect(
        service.resume(MAESTRO_TEST_CONSTANTS.CASE_INSTANCE_ID, MAESTRO_TEST_CONSTANTS.FOLDER_KEY),
      ).rejects.toThrow(TEST_CONSTANTS.ERROR_MESSAGE);
    });
  });
  ...
  describe('reopen', () => {
    ...
    it('should handle API errors', async () => {
      ...
      await expect(
        service.reopen(MAESTRO_TEST_CONSTANTS.CASE_INSTANCE_ID, MAESTRO_TEST_CONSTANTS.FOLDER_KEY, options),
      ).rejects.toThrow(TEST_CONSTANTS.ERROR_MESSAGE);
    });
  });
  ...
  describe('getExecutionHistory', () => {
    it('should return execution history for case instance with proper transformation', async () => {
      ...
      expect(mockApiClient.get).toHaveBeenCalledWith(MAESTRO_ENDPOINTS.CASES.GET_ELEMENT_EXECUTIONS(instanceId), {
        headers: expect.objectContaining({
          [FOLDER_KEY]: folderKey,
        }),
      });
    ...
    it('should handle API errors', async () => {
      ...
      await expect(
        service.getExecutionHistory(MAESTRO_TEST_CONSTANTS.CASE_INSTANCE_ID, MAESTRO_TEST_CONSTANTS.FOLDER_KEY),
      ).rejects.toThrow(TEST_CONSTANTS.ERROR_MESSAGE);
    });
  });
  ...
  describe('getStages', () => {
    ...
    it('should return case stages with tasks, SLA, and execution data', async () => {
      ...
      mockApiClient.get.mockResolvedValueOnce(mockExecutionHistory).mockResolvedValueOnce(mockCaseJson);
    ...
    it('should handle edge cases gracefully', async () => {
      ...
      mockApiClient.get.mockResolvedValueOnce(createMockCaseInstanceExecutionHistory()).mockResolvedValueOnce(null);
      ...
      expect(mockTaskService.getAll).toHaveBeenCalledWith(
        expect.objectContaining({
          filter: expect.stringMatching(
            new RegExp(`.*${caseInstanceId}.*${existingFilter.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}.*`),
          ),
          expand: expect.any(String),
        }),
      );
