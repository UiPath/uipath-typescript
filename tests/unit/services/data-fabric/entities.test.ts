...
describe("EntityService Unit Tests", () => {
  ...
  describe("queryRecordsById", () => {
    ...
    it("should include aggregates and groupBy in excludeFromPrefix so OData $ prefix is not added", async () => {
      ...
      expect(capturedConfig.excludeFromPrefix).toEqual(
        expect.arrayContaining(["aggregates", "groupBy"]),
      );
    });

    it("should send aggregate function values as uppercase enum strings", async () => {
      let capturedOptions: any;
      vi.mocked(PaginationHelpers.getAll).mockImplementation(async (_config, opts) => {
      ...
      expect(capturedOptions.aggregates.map((a: any) => a.function)).toEqual([
        "COUNT",
        "AVG",
        "MIN",
        "MAX",
