...
describe("EntityService Unit Tests", () => {
  ...
  describe("queryRecordsById", () => {
    ...
    it("should serialize valueList filters into the value form on join queries", async () => {
      vi.mocked(PaginationHelpers.getAll).mockResolvedValue({ items: [], totalCount: 0 });
      mockEntityNameLookup("Order");
      ...
      await entityService.queryRecordsById(ENTITY_TEST_CONSTANTS.ENTITY_ID, {
        ...
        filterGroup: {
          ...
          filterGroups: [
            {
              queryFilters: [
                { fieldName: "tier", operator: QueryFilterOperator.NotIn, valueList: ["bronze"] },
              ],
            },
          ],
        },
      ...
      const [, downstream] = vi.mocked(PaginationHelpers.getAll).mock.calls[0];
      // The multi-entity parser reads only `value` — valueList must be
      // serialized to a JSON-array string, including in nested groups.
      expect(downstream).toMatchObject({
        filterGroup: {
          queryFilters: [
          ...
          filterGroups: [
            {
              queryFilters: [{ fieldName: "tier", operator: "not in", value: '["bronze"]' }],
            },
          ],
        },
      });
    ...
    it("should pass valueList filters through unchanged when no joins are supplied", async () => {
      vi.mocked(PaginationHelpers.getAll).mockResolvedValue({ items: [], totalCount: 0 });
      ...
      // The single-entity routes accept valueList natively — no translation.
      expect(PaginationHelpers.getAll).toHaveBeenCalledWith(
        expect.anything(),
        expect.objectContaining({
          filterGroup: {
            queryFilters: [
              { fieldName: "region", operator: "in", valueList: ["east", "west"] },
            ],
          },
        }),
      );
