...
describe('PaginationHelpers Unit Tests', () => {
  ...
  describe('parseCursor', () => {
    ...
    it('should throw error for invalid base64 string', () => {
      ...
      expect(() => PaginationHelpers.parseCursor(invalidCursor)).toThrow(
        PAGINATION_TEST_CONSTANTS.ERROR_INVALID_CURSOR,
      );
    });
    ...
    it('should throw error for valid base64 but invalid JSON', () => {
      ...
      expect(() => PaginationHelpers.parseCursor(invalidJson)).toThrow(PAGINATION_TEST_CONSTANTS.ERROR_INVALID_CURSOR);
    });
  });

  describe('validateCursor', () => {
    it('should not throw error when no cursor is provided', () => {
      expect(() => PaginationHelpers.validateCursor({})).not.toThrow();
    });
    ...
    it('should not throw error for valid cursor with matching pagination type', () => {
      ...
      expect(() => PaginationHelpers.validateCursor(options, PaginationType.OFFSET)).not.toThrow();
    });

    it('should throw error when cursor is null', () => {
      const options = { cursor: null as any };

      expect(() => PaginationHelpers.validateCursor(options)).toThrow(
        PAGINATION_TEST_CONSTANTS.ERROR_CURSOR_MUST_BE_VALID,
      );
    });
    ...
    it('should throw error for invalid cursor format', () => {
      ...
      expect(() => PaginationHelpers.validateCursor(options)).toThrow(
        PAGINATION_TEST_CONSTANTS.ERROR_INVALID_CURSOR_FORMAT,
      );
    });
    ...
    it('should throw error when cursor is missing pagination type but type validation is requested', () => {
      ...
      expect(() => PaginationHelpers.validateCursor(options, PaginationType.OFFSET)).toThrow(
        PAGINATION_TEST_CONSTANTS.ERROR_INVALID_CURSOR_MISSING_TYPE,
      );
    });
    ...
    it('should throw error when cursor pagination type does not match expected type', () => {
      ...
      expect(() => PaginationHelpers.validateCursor(options, PaginationType.OFFSET)).toThrow(
        PAGINATION_TEST_CONSTANTS.ERROR_PAGINATION_TYPE_MISMATCH,
      );
    });
  });
  ...
  describe('validatePaginationOptions', () => {
    it('should successfully validate valid pagination options with pageSize', () => {
      ...
      const result = PaginationHelpers.validatePaginationOptions(options, PaginationType.OFFSET);
    ...
    it('should throw error when pageSize is negative', () => {
      ...
      expect(() => PaginationHelpers.validatePaginationOptions(options, PaginationType.OFFSET)).toThrow(
        PAGINATION_TEST_CONSTANTS.ERROR_PAGE_SIZE_POSITIVE,
      );
    });
    ...
    it('should throw error when jumpToPage is negative', () => {
      ...
      expect(() => PaginationHelpers.validatePaginationOptions(options, PaginationType.OFFSET)).toThrow(
        PAGINATION_TEST_CONSTANTS.ERROR_JUMP_TO_PAGE_POSITIVE,
      );
    });
    ...
    it('should throw error when jumpToPage is used with token-based pagination', () => {
      ...
      expect(() => PaginationHelpers.validatePaginationOptions(options, PaginationType.TOKEN)).toThrow(
        PAGINATION_TEST_CONSTANTS.ERROR_JUMP_TO_PAGE_NOT_SUPPORTED,
      );
    });
  });
  ...
  describe('getRequestParameters', () => {
    ...
    it('should return parameters for first page request with OFFSET pagination', () => {
      ...
      const result = PaginationHelpers.getRequestParameters(options, PaginationType.OFFSET);
    ...
    it('should return parameters for first page request with TOKEN pagination', () => {
      ...
      const result = PaginationHelpers.getRequestParameters(options, PaginationType.TOKEN);
    ...
    it('should throw error for invalid cursor', () => {
      ...
      expect(() => PaginationHelpers.getRequestParameters(options)).toThrow(
        PAGINATION_TEST_CONSTANTS.ERROR_INVALID_CURSOR,
      );
    });
  ...
  describe('getAllNonPaginated', () => {
    ...
    it('should fetch all items without pagination and return transformed items', async () => {
      const apiResponse = createMockApiResponse(MOCK_RAW_ITEMS, PAGINATION_TEST_CONSTANTS.TOTAL_COUNT_100);
  ...
  describe('getAll', () => {
    ...
    beforeEach(() => {
      ...
      mockConfig = {
        ...
        getEndpoint: (folderId?: number) =>
          folderId
            ? `${PAGINATION_TEST_CONSTANTS.ENDPOINT_API_ITEMS}/${folderId}`
            : PAGINATION_TEST_CONSTANTS.ENDPOINT_API_ITEMS,
        getByFolderEndpoint: PAGINATION_TEST_CONSTANTS.ENDPOINT_API_FOLDERS_ITEMS,
        transformFn: mockTransformFunction,
        pagination: {
    ...
    it('should use non-paginated flow when no pagination parameters provided', async () => {
      const apiResponse = createMockApiResponse(MOCK_RAW_ITEM, PAGINATION_TEST_CONSTANTS.TOTAL_COUNT_100);
    ...
    it('should use paginated flow when cursor is provided', async () => {
      const cursor = createMockPaginationCursor();
      const mockResponse = createMockPaginatedResponse(MOCK_RAW_ITEM, { hasNextPage: true });
