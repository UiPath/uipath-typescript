...
- **Deploy says the app is "already deployed" and won't upgrade** — an app with that name was deployed from a *different* project; the CLI can only upgrade deployments made from the same registration. Publish and deploy under a new name (or delete the existing deployed app first, either [in Orchestrator](https://docs.uipath.com/orchestrator/automation-cloud/latest/user-guide/managing-apps#deleting-a-deployed-app) or with [`uip codedapp delete`](../coded-apps/cli-reference.md#delete)).
