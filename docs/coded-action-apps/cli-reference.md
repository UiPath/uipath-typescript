...
!!! note
    Publishing and deploying coded action apps with `client-id` and `client-secret` based login is not currently supported. Use interactive browser-based user OAuth login only.
...
| `-t, --type` | string | App type: `Web` or `Action`. Use `Action` for coded action apps | `Web` |
