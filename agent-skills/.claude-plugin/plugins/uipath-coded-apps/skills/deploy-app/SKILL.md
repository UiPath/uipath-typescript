...
- **If `uipath` is found** → check the version output. It should contain `latest`. If the version is different, uninstall and reinstall:
  ```bash
  npm uninstall -g @uipath/uipath-ts-cli
  ```
  Then go to step 0b.
- **If version matches `latest`** → skip to Step 1.
...
```bash
npm install -g @uipath/uipath-ts-cli@latest
```
