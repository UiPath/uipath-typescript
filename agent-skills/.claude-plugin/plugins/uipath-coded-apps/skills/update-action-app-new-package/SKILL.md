...
- If `<existing-client-id>` is **empty**, ask:
  ...
  >    (replace `<orgId>` and `<tenantId>` with your actual org slug and tenant name)
...
> `https://cloud.uipath.com/<orgId>/<tenantId>/actions_`
>
> Replace `<orgId>` and `<tenantId>` with your actual organzation id and tenant id. Without the correct redirect URI the OAuth flow will fail when the app loads in Action Center.
