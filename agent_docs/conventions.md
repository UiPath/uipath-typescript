...
- **Barrel files must use `export * from`**, not `export type * from`. Using `export type *` re-exports only type declarations and silently drops runtime values (class constructors, enums), causing `undefined` errors for SDK consumers who import them. **Exception for named re-exports under `isolatedModules: true`**: when a tsconfig has `isolatedModules: true`, TypeScript requires `export type { Name }` for type-only named re-exports (plain `export { Name }` raises TS1205 for type declarations). In that case, `export type { Name }` is required — the prohibition targets the wildcard form `export type * from`, not individual named `export type { X }` re-exports.
