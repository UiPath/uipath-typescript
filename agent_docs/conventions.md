...
- **When a public method signature changes, update the corresponding interface declaration** — if `IUiPath`, `{Entity}ServiceModel`, or any other interface declares the method with its old signature, callers holding an interface reference will get a TypeScript error when they try to pass the new parameter or overload. The interface and implementation must be kept in sync on every signature change, including new overloads.
