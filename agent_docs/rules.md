...
- **NEVER** test client-side `ValidationError` in integration tests — a call that throws before making any HTTP request (e.g., `updateValueById(0, ...)` failing on invalid ID) belongs in unit tests, not integration tests. Integration tests must only exercise code paths that reach the actual API.
