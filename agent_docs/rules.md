...
- **`getAll` and `getById` need separate transform completeness tests** — each has its own `transformFn` closure, so a `getById` test doesn't cover `getAll`. A bug in one closure slips through unless both are tested.
