...
- **Validate transform completeness** — for any method with a transform pipeline, verify both: (a) transformed fields have correct values (`result.createdTime`), AND (b) original PascalCase fields are absent (`(result as any).CreationTime` is `undefined`). This catches transform regressions that value-only assertions miss. Existing pattern: Assets (`assets.test.ts:94`), Queues (`queues.test.ts:88`), ChoiceSets (`choicesets.test.ts:243`). **For semantic rename tests, use a distinctive non-null value in the mock override** (e.g., a specific timestamp string) — a `null` default doesn't verify value preservation: `expect(result.renamedField).toBe(null)` passes whether the rename correctly transferred the null or the field was never populated. A distinctive value like `'2000-01-01T00:00:00.000Z'` confirms the specific value traveled through the rename intact. **Mocks for transform completeness tests must provide raw API wire-format field names, not post-transform SDK field names** — if the mock already uses the SDK field name (e.g., `expiresTime` instead of `expiresAt`), the rename transform never runs and `expect((result as any).expiresAt).toBeUndefined()` passes vacuously. The "original field is absent" assertion is only meaningful when the mock supplies the wire name.
- **`getAll` and `getById` need separate transform completeness tests** — each has its own `transformFn` closure, so a `getById` test doesn't cover `getAll`. A bug in one closure slips through unless both are tested. **This extends to overloads of the same method** — when `getById` has multiple overloads (e.g., positional `getById(id, folderId)` and options-form `getById(id, options?)`), each dispatch branch in the implementation has its own transform call, and a completeness test on the positional overload does not exercise the options-object branch. Both branches need their own completeness test.
...
- **When unit-testing error paths for methods that use raw `fetch()`, mock with `new Response(...)` not a plain object literal** — the error response parser calls `response.clone().json()`, which plain objects don't support. A plain literal silently routes to the parser's fallback path, so the test appears to pass without exercising the intended code path. Use: `fetchMock.mockResolvedValue(new Response(JSON.stringify({ message: '...' }), { status: 403, headers: { 'content-type': 'application/json' } }))`.
- **NEVER** wrap integration test API calls in try/catch — let errors propagate naturally. Silent catches mask real failures and make tests pass when they should fail. **Exception: polling loops that handle transient visibility errors** (e.g., "not yet visible in PIMS") may use try-catch, but scope the catch to **only** the specific call that can transiently fail. Keep state-changing operations (e.g., `retry()`, `cancel()`) outside the catch so their failures propagate naturally. A broad catch that swallows both the polling call and a state-change call will exhaust retries with a misleading "did not reach X within Ys" error instead of surfacing the real cause.
...
- [ ] `release-metadata.json` — regenerated by `release-metadata:gen` (auto-runs on the version-bump PR). No manual edit; if you added a service/method, confirm it appears after the bump. `@internal` members are excluded.
