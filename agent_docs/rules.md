...
- **When adding a new resource type to `ResourceRegistry` in `tests/integration/utils/cleanup.ts`, also add the corresponding cleanup helper and delete loop in `cleanupAllTestResources()`** — a new registry slot without a matching cleanup entry means `registerResource()` calls are inert and orphaned resources from a mid-run crash are never deleted by global cleanup.
