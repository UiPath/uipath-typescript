...
- **Validate transform completeness** — for any method with a transform pipeline, verify both: (a) transformed fields have correct values (`result.createdTime`), AND (b) original PascalCase fields are absent (`(result as any).CreationTime` is `undefined`). This catches transform regressions that value-only assertions miss. Existing pattern: Assets (`assets.test.ts:94`), Queues (`queues.test.ts:88`), ChoiceSets (`choicesets.test.ts:243`). **For semantic rename tests, use a distinctive non-null value in the mock override** (e.g., a specific timestamp string) — a `null` default in the mock means the assertion `expect(result.renamedField).toBe(null)` passes even when the rename is missing (the field is absent/`undefined`, which can slip past in some equality patterns). Use `expect(result.renamedField).toBe('2026-06-01T00:00:00.000Z')` with a matching mock override to confirm the value actually flowed through the rename.
