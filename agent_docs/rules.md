...
- When adding methods, update `docs/oauth-scopes.md` with required OAuth scopes. **NEVER** skip this — missing scopes break the OAuth integration guide. **Exception: `@internal` methods do not get an entry** — they are not part of the public API surface and do not appear in the OAuth guide. Similarly, `mkdocs.yml` nav and the docs site do not get entries for services or methods marked `@internal`.
...
- [ ] `mkdocs.yml` — nav entry added (if new service, not needed for methods on existing services; skip for `@internal`-only services)
