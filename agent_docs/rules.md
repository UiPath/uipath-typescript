...
- **NEVER** wrap integration test API calls in try/catch — let errors propagate naturally. Silent catches mask real failures and make tests pass when they should fail. **Exception: polling loops that handle transient visibility errors** (e.g., "not yet visible in PIMS") may use try-catch, but scope the catch to **only** the specific call that can transiently fail. For Data Fabric resources, use the readiness helpers in `tests/integration/utils/helpers.ts` (`createEntityAwaitingReady`, `awaitEntityReady`, `awaitRecordVisible`) instead of hand-rolled polls — newly created entities/records propagate asynchronously (authorization records, query indexes, backing tables), and operating on them immediately races that propagation. Keep state-changing operations (e.g., `retry()`, `cancel()`) outside the catch so their failures propagate naturally. A broad catch that swallows both the polling call and a state-change call will exhaust retries with a misleading "did not reach X within Ys" error instead of surfacing the real cause.
