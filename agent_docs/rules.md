...
- **`getAll` and `getById` require separate transform completeness tests** — each method uses its own `transformFn` closure; a passing transform test on `getById` does not validate `getAll`'s transform. A bug introduced into `getAll`'s closure will silently slip through unless `getAll` has its own test asserting camelCase fields are present and PascalCase fields are absent.
