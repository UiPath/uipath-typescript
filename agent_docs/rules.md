...
- **`getAll` and `getById` need separate transform completeness tests** — each has its own `transformFn` closure, so a `getById` test doesn't cover `getAll`. A bug in one closure slips through unless both are tested. **This extends to overloads of the same method** — when `getById` has multiple overloads (e.g., positional `getById(id, folderId)` and options-form `getById(id, options?)`), each overload has its own closure and needs its own completeness test. A passing completeness test on the positional overload does not cover the options-object overload.
