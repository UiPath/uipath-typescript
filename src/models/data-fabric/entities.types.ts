...
export interface EntityUpsertResponse extends EntityRecord {
  /**
   * What was written at each record, mirroring the nesting of the request.
   * Present only when the payload nested child records.
   */
...
export interface EntityUpsertOptions extends EntityFolderScopedOptions {
  /**
   * Level of entity expansion (default: 0). Applies to a single-record upsert; a write that
   * spans related entities ignores it.
   */
  expansionLevel?: number;
}
