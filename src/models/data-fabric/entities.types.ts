...
/**
 * What a multi-entity transactional upsert did to one record.
 */
export enum EntityMultiEntityWriteOperation {
  /** The record carried no `Id`, so a new one was created. */
  Insert = "Insert",
  /** The record named itself with an `Id`, so that row was updated. */
  Update = "Update",
}

/**
 * Result for a single record written by a multi-entity transactional upsert.
 * Mirrors the request, so each node can be matched back to the record that produced it.
 */
export interface EntityMultiEntityWriteResponseNode {
  ...
  /** Record ID — generated by the service for an insert */
  id: string;
...
/**
 * Root of the per-record results for a multi-entity transactional upsert.
 */
...
/**
 * Response from a multi-entity transactional upsert.
 *
 * The transaction reports what happened per record rather than echoing field values, so
 * unlike {@link EntityInsertResponse} this response carries no entity field data.
 */
export interface EntityUpsertMultiEntityTransactionResponse {
  /** Record ID of the root record */
  Id: string;
  /** What the transaction wrote, one node per record, mirroring the request */
  transaction: EntityMultiEntityWriteResponse;
}

/**
 * Options for a multi-entity transactional upsert
 */
export interface EntityUpsertMultiEntityTransactionOptions extends EntityFolderScopedOptions {}
