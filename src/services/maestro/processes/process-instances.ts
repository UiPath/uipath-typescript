...
import {
  ProcessInstanceMap,
  ProcessInstanceExecutionHistoryMap,
} from '../../../models/maestro/process-instances.constants';
...
export class ProcessInstancesService extends BaseService implements ProcessInstancesServiceModel {
  ...
  async getExecutionHistory(instanceId: string): Promise<ProcessInstanceExecutionHistoryResponse[]> {
    const response = await this.get<ProcessInstanceExecutionHistoryResponse[]>(
  ...
  async cancel(
    ...
    const response = await this.post<ProcessInstanceOperationResponse>(
  ...
  async getIncidents(instanceId: string, folderKey: string): Promise<ProcessIncidentGetResponse[]> {
    const rawResponse = await this.get<any[]>(MAESTRO_ENDPOINTS.INCIDENTS.GET_BY_INSTANCE(instanceId), {
      headers: createHeaders({ [FOLDER_KEY]: folderKey }),
    });
