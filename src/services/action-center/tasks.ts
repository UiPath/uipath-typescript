...
import {
  TASK_TYPE_ENDPOINTS,
  TaskAssignmentResponseCollection,
  TaskGetFormOptions,
  TasksAssignOptions,
...
export class TaskService extends BaseService implements TaskServiceModel {
  ...
  async create(task: TaskCreateOptions, folderId: number): Promise<TaskCreateResponse> {
    ...
    const response = await this.post<TaskCreateResponse>(TASK_ENDPOINTS.CREATE_GENERIC_TASK, externalTask, { headers });
  ...
  > {
    ...
    const transformUserResponse = (user: any) => pascalToCamelCaseKeys(user) as UserLoginInfo;
  ...
  async getById(id: number, options: TaskGetByIdOptions = {}, folderId?: number): Promise<TaskGetResponse> {
    ...
    const response = await this.get<TaskGetResponse>(TASK_ENDPOINTS.GET_BY_ID(id), {
      params: apiOptions,
      headers,
    });
  ...
  @track('Tasks.Assign')
  async assign(
    taskAssignments: TaskAssignmentOptions | TaskAssignmentOptions[],
    ...
    const response = await this.post<TaskAssignmentResponseCollection>(TASK_ENDPOINTS.ASSIGN_TASKS, pascalOptions);
  ...
  async reassign(
    ...
    const response = await this.post<TaskAssignmentResponseCollection>(TASK_ENDPOINTS.REASSIGN_TASKS, pascalOptions);
  ...
  async unassign(
  ...
  ): Promise<OperationResponse<{ taskId: number }[] | TaskAssignmentResponse[]>> {
    // Normalize input to array
    ...
    const response = await this.post<TaskAssignmentResponseCollection>(TASK_ENDPOINTS.UNASSIGN_TASKS, options);
    ...
    return processODataArrayResponse(
      transformedResponse,
      taskIdArray.map((id) => ({ taskId: id })),
    );
  }
  ...
  private getByTaskType(
    ...
    const extraParams: TaskGetFormOptions =
      taskType === TaskType.Form ? { expandOnFormLayout: true, ...options } : options;
   ...
   * @returns Promise resolving to the task
   */
  private async getTaskByTypeEndpoint(
    id: number,
    folderId: number,
    endpoint: string,
    extraParams: TaskGetFormOptions = {},
  ): Promise<TaskGetResponse> {
    const headers = createHeaders({ [FOLDER_ID]: folderId });

    const response = await this.get<TaskGetResponse>(endpoint, {
      params: {
        taskId: id,
        ...extraParams,
      },
