...
| `check-jsdoc-sync.mjs` | JSDoc on `{Entity}ServiceModel` identical to the service class copy (agent_docs/rules.md). Pairing uses TypeScript AST `implements XServiceModel`; doc-only scoped models are skipped |
| `check-docs-consistency.mjs` | Every non-`@internal` `@track` method appears in the matching `docs/oauth-scopes.md` section; scope cells contain scopes, not prose. Source parsing uses TypeScript AST decorators/methods; Markdown parsing is section-aware |
