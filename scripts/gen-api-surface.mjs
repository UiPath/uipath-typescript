#!/usr/bin/env node
// Writes a signature-level snapshot of the public API to the given path.
...
const OUT = process.argv[3] ? resolve(process.argv[3]) : fail('output path is required');


function readAliases() {
  const p = resolve(ROOT, 'tsconfig.json');
  if (!existsSync(p)) return [];
  // ts.parseConfigFileTextToJson handles JSONC; hand-rolled comment stripping
  // trips over strings like "@/*". Swallowing a parse error here would drop
  // every aliased re-export from the snapshot.
  const parsed = ts.parseConfigFileTextToJson(p, readFileSync(p, 'utf8'));
  if (parsed.error) fail(`tsconfig.json could not be parsed for path aliases`);
...
const isFile = (p) => {
  try { return statSync(p).isFile(); }
    ...
    // A missing candidate is normal; anything else would otherwise surface as
    // a misleading "cannot resolve" further up.
    if (e.code !== 'ENOENT') console.warn(`api-surface: cannot stat ${p}: ${e.message}`);
    return false;
  }
};
...
// Decorators, `export`, `async` and `public` are not part of the call contract.
const stripNoise = (s) =>
...
// Slicing at the first `{` would clip a default value such as `options = {}`.
function sigOf(n) {
