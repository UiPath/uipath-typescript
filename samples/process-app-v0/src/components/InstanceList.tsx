        ...
        >
          ...
          <div
            className={`absolute left-0 top-4 bottom-4 w-1 rounded-r-full ${
              instance.latestRunStatus?.toLowerCase() === 'completed'
                ? 'bg-green-400'
                : instance.latestRunStatus?.toLowerCase() === 'running'
                  ? 'bg-blue-400'
                  : instance.latestRunStatus?.toLowerCase() === 'failed'
                    ? 'bg-red-400'
                    : 'bg-gray-300'
            }`}
          ></div>
          ...
          <div className="flex items-start justify-between mb-3 ml-3">
            ...
            <span
              className={`px-3 py-1 text-xs font-semibold rounded-full shadow-sm ${getStatusColor(instance.latestRunStatus || 'unknown')}`}
            >
          ...
          <div className="space-y-2 text-sm text-gray-600 ml-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-mono">
                {new Date(instance.startedTime).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}{' '}
                {new Date(instance.startedTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
              </span>
            </div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-mono bg-gray-100 px-2 py-1 rounded">
                {formatDuration(instance.startedTime, instance.completedTime || undefined)}
              </span>
            </div>
            {instance.completedTime && (
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-mono">
                  {new Date(instance.completedTime).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}{' '}
                  {new Date(instance.completedTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                </span>
              </div>
            )}
          </div>
