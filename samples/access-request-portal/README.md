...
1. Create an **external application** (confidential or public with PKCE) in UiPath Admin with redirect URI `http://localhost:5173` and scopes `PM.User PM.Group PM.Directory.Read offline_access`. The Roles service endpoints are governed by the caller's platform roles rather than OAuth scopes.
