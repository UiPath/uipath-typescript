...
A collection of UiPath **Coded Action App** samples, built with the `@uipath/coded-action-app` and `@uipath/uipath-typescript` SDKs. Most samples implement the same **Loan Application Review** action — a reviewer assesses an applicant and completes the task with an **Approve** or **Reject** decision — and each one demonstrates a different way of bringing the supporting document or data into the app.
...
| [`action-app-with-functions`](./action-app-with-functions) | The reviewer needs a **credential the browser cannot read** (a Credential or Secret asset) | Invoking a **coded function** with typed input/output to resolve an asset as a robot, plus listing assets via `Assets` | `OR.Default`, `OR.Assets.Read` |
...
The four data-backed samples (file attachment, storage bucket, data fabric, functions) additionally require a non-confidential **External Application** (OAuth client) with the scopes listed above. See the individual sample README for the exact registration steps.
