...
The four samples that call a UiPath service (file attachment, storage bucket, data fabric, document validation) additionally require a non-confidential **External Application** (OAuth client) with the scopes listed above. See the individual sample README for the exact registration steps.
