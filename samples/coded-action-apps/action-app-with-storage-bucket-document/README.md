# Action App With Storage Bucket Document

A UiPath Coded Action App template for **Loan Application Review** that loads its document from a **Storage Bucket**. Reviewers can assess an applicant's details, preview and download a PDF document fetched from a Storage Bucket, and complete the task with an Approve or Reject decision.
...
| `loanDocumentFilePath` | string | No | Path to the loan document (PDF) within the Storage Bucket |
...
1. Import the [Template With Storage Bucket Doc.uis](./Template%20With%20Storage%20Bucket%20Doc.uis) solution in **Studio Web**.
...
2. **Document tab** — On first visit, resolves the Storage Bucket by name, fetches a signed read URI for the file at the provided path, and renders the PDF inline with:
