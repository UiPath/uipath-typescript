    ...
    if (activeTab === 'document' && !hasLoadedDocument && !isLoadingDocument && formData) {
      ...
      const loadDocument = async () => {
        try {
          ...
          // Escape single quotes for the OData filter by doubling them, so bucket
          // names like "O'Brien's Docs" produce a valid query instead of a 400.
          const escapedBucketName = formData.loanDocumentStorageBucket.replace(/'/g, "''");
          const bucketsResponse = await uipath.bucketService.getAll({
            filter: `name eq '${escapedBucketName}'`
          });

          const bucket = bucketsResponse.items.find((b) => b.name === formData.loanDocumentStorageBucket);
