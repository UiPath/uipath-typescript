            ...
            <tbody>
              {conversationList.map(conv => (
                <tr key={conv.id} className="border-t hover:bg-gray-50">
                  ...
                  <td className="px-2 py-1">{new Date(conv.createdTime).toLocaleString()}</td>
                  <td className="px-2 py-1">
                    <button onClick={() => deleteConversation(conv.id)} className="text-red-600 hover:underline">Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
