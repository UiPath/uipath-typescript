...
function findNextFence(lowerContent: string, pos: number): { openIdx: number; blockType: 'html' | 'latex' } | null {
...
function SandboxedHtml({ html }: { html: string }) {
  ...
  return (
    <div className="my-2 border border-white/10 rounded-lg overflow-hidden">
      ...
      {showRaw ? (
        <pre
          className="p-3 overflow-auto text-xs text-gray-300 whitespace-pre-wrap break-all"
...
function RenderedLatex({ latex }: { latex: string }) {
  ...
  return <div className="my-2 overflow-x-auto" dangerouslySetInnerHTML={{ __html: renderedHtml! }} />;
}
...
function CodeCopyButton({ text }: { text: string }) {
  ...
  return (
    <button onClick={handleCopy} className="px-2 py-1 text-xs bg-white/10 hover:bg-white/20 rounded transition-colors">
      {copied ? 'Copied!' : 'Copy'}
    </button>
  );
}
...
function MessageActions({
  ...
  return (
    <div className="flex items-center gap-2 mt-3 opacity-0 hover:opacity-100 transition-opacity">
      ...
      >
        <svg
          className={`w-4 h-4 ${message.feedbackRating === 'positive' ? 'text-green-400' : 'text-gray-400'}`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
      ...
      >
        <svg
          className={`w-4 h-4 ${message.feedbackRating === 'negative' ? 'text-red-400' : 'text-gray-400'}`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
    ...
    return (
      <div className="rounded-lg overflow-hidden max-w-md">
        <img
          ...
          alt="Visual content generated by assistant"
    ...
    <div className={`px-6 py-4 ${isUser ? 'bg-transparent' : 'bg-chat-input/30'}`}>
      <div className="flex gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          {message.citations && message.citations.length > 0 && <CitationsList citations={message.citations} />}
  ...
  return (
    <div className={`border rounded-lg overflow-hidden ${statusColor}`}>
      ...
      >
        ...
        {!toolCall.isComplete && <Spinner className="w-3 h-3 border-current" />}
        <svg
        ...
        <div className="flex-1">
          ...
          {confirmValue?.inputValue != null && (
            <pre className="mt-1 p-2 bg-black/20 rounded text-xs overflow-x-auto whitespace-pre-wrap break-all text-gray-300">
              {formatJson(
                typeof confirmValue.inputValue === 'string'
                  ? confirmValue.inputValue
                  : JSON.stringify(confirmValue.inputValue),
              )}
            </pre>
          )}
