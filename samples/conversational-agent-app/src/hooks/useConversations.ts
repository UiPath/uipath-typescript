...
export function useConversations(
  ...
  const updateLabel = useCallback((id: string, label: string) => {
    setConversations((prev) => prev.map((c) => (c.id === id ? ({ ...c, label } as ConversationGetResponse) : c)));
    setCurrentConversation((prev) => (prev?.id === id ? ({ ...prev, label } as ConversationGetResponse) : prev));
  }, []);
