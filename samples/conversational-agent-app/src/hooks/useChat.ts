...
function updateMessage(setMessages: SetMessagesFn, id: string, updates: Partial<ChatMessage>) {
  setMessages((prev) => prev.map((m) => (m.id === id ? { ...m, ...updates } : m)));
}

function updateMessageWith(setMessages: SetMessagesFn, id: string, updater: (m: ChatMessage) => Partial<ChatMessage>) {
...
function handleToolCallComplete(
  ...
  const output = typeof endEvent.output === 'string' ? endEvent.output : JSON.stringify(endEvent.output ?? '');
  setMessages((prev) =>
    prev.map((m) =>
      m.id === assistantId
        ? {
            ...
            toolCalls: (m.toolCalls || []).map((tc) =>
              tc.toolCallId === toolCallId ? { ...tc, output, isError: endEvent.isError, isComplete: true } : tc,
            ),
          }
        : m,
    ),
...
function handleImageContent(part: any, assistantId: string, exchangeId: string, setMessages: SetMessagesFn) {
  let imageChunks = '';
...
function setupAssistantHandlers(
  ...
  message.onToolCallStart((toolCall) => {
    const info: ToolCallInfo = {
      ...
      input:
        typeof toolCall.startEvent.input === 'string'
...
export function useChat(
  ...
  const ensureSession = useCallback((): Promise<void> => {
    ...
    setupExchangeHandlers(session, exchangeAssistantIdRef, setMessages, pendingInterruptRef, setIsStreaming, setError);
  ...
  const resolveInterrupt = useCallback(
    async (messageId: string, approved: boolean) => {
      ...
      try {
        ...
        setMessages((prev) =>
          prev.map((m) =>
            m.id === messageId && m.interrupt ? { ...m, interrupt: { ...m.interrupt, resolved: true, approved } } : m,
          ),
        );
      } catch (err) {
  ...
  const startExchange = useCallback(
    async (content: string, assistantMessageId: string, attachments: ConversationAttachmentUploadResponse[] = []) => {
  ...
  const sendMessage = useCallback(
    async (content: string, attachments?: File[]) => {
      ...
      if (attachments && attachments.length > 0) {
        try {
          uploaded = await Promise.all(attachments.map((file) => conversation.uploadAttachment(file)));
        } catch (err) {
