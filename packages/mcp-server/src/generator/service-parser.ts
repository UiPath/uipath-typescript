...
import { Project, SourceFile, ClassDeclaration, MethodDeclaration, Type, Node, SyntaxKind } from 'ts-morph';
...
/**
 * Helper to convert service name to tool name prefix
 */
export function serviceNameToToolPrefix(serviceName: string, category: string): string {
 ...
 */
export function methodToToolName(
  methodName: string,
  serviceName: string,
  category: string,
  separator: string = '_'
): string {
  const prefix = serviceNameToToolPrefix(serviceName, category);
  return `${prefix}${separator}${methodName}`;
}
