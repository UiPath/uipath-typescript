...
export function readConfig(options: ReadConfigOptions = {}): UiPathConfig {
  ...
  // Detect dev mode from environment - works across all bundlers
  ...
  if (!fs.existsSync(fullPath)) {
    if (isDev) {
      ...
      throw createConfigError(
        ...
        `For local development, create a ${DEFAULT_CONFIG_PATH} file with:\n` +
        JSON.stringify(SAMPLE_CONFIG, null, 2)
      )
    } else {
      // In prod build, just log info - deployment will inject config
      console.log(`[${PLUGIN_NAME}] ℹ️  No ${DEFAULT_CONFIG_PATH} found - config will be injected at deployment`)
...
function getMetaTagName(configKey: string): string {
  const suffix = CONFIG_TO_META_TAG[configKey]
