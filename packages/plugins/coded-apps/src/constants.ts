 ...
 * Derived from UIPATH_META_TAGS as single source of truth.
 */
export const REQUIRED_KEYS_DEV_LIST = Object.keys(UIPATH_META_TAGS) as ValidConfigKey[]
...
export const MESSAGES = {
  ...
  MISSING_DEV_KEYS: (keys: string[]) =>
    `⚠️  Missing keys for local development: ${keys.join(', ')}`,

  // File errors
  CONFIG_NOT_FOUND: (path: string) => `❌ Config file not found: ${path}`,
  CONFIG_CREATE_HINT: (configPath: string, sample: string) =>
    `For local development, create a ${configPath} file with:\n${sample}`,
