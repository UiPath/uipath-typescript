...
import { promptProjectSelection } from '../utils/project-selection.js';
...
export async function executePush(options: PushOptions): Promise<void> {
  ...
  if (!projectId) {
    ...
    const { action } = await inquirer.prompt<{ action: string }>([{
      type: 'list',
      name: 'action',
      message: 'No project ID found. What would you like to do?',
      choices: [
        { name: 'Create a new Coded App project', value: 'create' },
    ...
    if (action === 'create') {
      ...
      projectId = await promptProjectSelection(envConfig, logger, 'push');
    }
  }
