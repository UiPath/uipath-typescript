/**
 * Auto-generated CLI Command: queues list
 *
...
export default class List extends Command {
  static description = "Gets all queues across folders with optional filtering and folder scoping\n\nThe method returns either:\n- An array of queues (when no pagination parameters are provided)\n- A paginated result with navigation cursors (when any pagination parameter is provided)";
  ...
  async run(): Promise<void> {
    ...
    const spinner = ora('Executing queues list...').start();

    try {
      const client = await getSDKClient();
      const result = await client.queues.getAll(flags as any);
