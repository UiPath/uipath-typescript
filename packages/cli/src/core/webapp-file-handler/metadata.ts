...
import type { WebAppPushConfig, ProjectFile, FileOperationPlan } from './types.js';
...
function createNewMetadataPayload(config: WebAppPushConfig): Record<string, unknown> {
  return {
    schemaVersion: DEFAULT_SCHEMA_VERSION,
    name: config.projectId,
    description: '',
    lastPushDate: new Date().toISOString(),
    email: getPushAuthorEmail(config.envConfig.accessToken, config.logger),
  };
}
 ...
 * Updates lastPushDate, email, and schemaVersion (from plan) and optionally codeVersion from remote.
 */
export async function prepareMetadataFileForPlan(
  ...
  try {
    ...
    if (err.code === 'ENOENT') {
      ...
      const remoteEntry =
        remoteFiles.get(PUSH_METADATA_REMOTE_PATH) ||
        remoteFiles.get(PUSH_METADATA_RELATIVE_PATH) ||
        remoteFiles.get(PUSH_METADATA_FILENAME);
      if (remoteEntry) {
        try {
          const remoteContent = await downloadRemoteFile(config, remoteEntry.id);
          const parsed = JSON.parse(remoteContent.toString('utf-8'));
  ...
  metadata.email = getPushAuthorEmail(config.envConfig.accessToken, config.logger);

  if (plan && !isNewMetadata) {
    metadata.schemaVersion = computeNextSchemaVersion(metadata.schemaVersion, plan);
  }

  // Optional: bump codeVersion from remote metadata when present (legacy/backward compatibility).
  ...
  if (remoteMetadata) {
    try {
      ...
      if (remoteMetadataObj?.codeVersion && typeof remoteMetadataObj.codeVersion === 'string') {
