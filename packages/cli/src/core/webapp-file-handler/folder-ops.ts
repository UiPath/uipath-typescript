...
export async function ensureContentRootExists(
  ...
  if (!currentFolders.has(remoteContentRoot)) {
    await api.createFolderAtRoot(config, bundlePath, lockKey);
    const next = await api.fetchRemoteStructure(config);
    setStructure(next);
    currentFolders = getRemoteFoldersMap(next);
    const distId = currentFolders.get(bundlePath)?.id;
    const sourceId = currentFolders.get(REMOTE_SOURCE_FOLDER_NAME)?.id;
    if (distId && sourceId) {
      await api.moveFolder(config, distId, sourceId, lockKey);
    }
    const after = await api.fetchRemoteStructure(config);
    setStructure(after);
  }
}
